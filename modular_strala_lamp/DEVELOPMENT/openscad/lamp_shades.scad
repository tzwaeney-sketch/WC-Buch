...
// 9. Organic S-curve
// Single closed profile: outer wall up, then inner wall back down.
module shade_organic_curve() {
    h=shade_height;
    botr=shade_bottom_diameter/2; topr=shade_top_diameter/2;
    w = shade_wall;
    N = 40;
    // outer wall bottom->top (S-curve, bulges outward mid-height)
    outer = [for(i=[0:N])
        let(t=i/N,
            base = topr + (botr-topr)*(0.5+0.5*cos(180*t)),
            bulge = 10*sin(180*t),
            rad = base + bulge)
        [rad, t*h]];
    // inner wall top->bottom (offset inward by wall)
    inner = [for(i=[N:-1:0])
        let(t=i/N,
            base = topr + (botr-topr)*(0.5+0.5*cos(180*t)),
            bulge = 10*sin(180*t),
            rad = max(0.2, base + bulge - w))
        [rad, t*h]];
    _shade_body(h)
        rotate_extrude($fn=128)
